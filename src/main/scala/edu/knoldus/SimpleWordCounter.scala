...
object ZioFileReader extends zio.App {
  ...
  val wordCounter: String => Task[Int] = (str: String) =>
    ZIO {
      str.split(" ").length
  }
  val fileWordCount: ZIO[Console, Throwable, Unit] =
    for {
      filesData: List[String] <- ZIO.foreach(filePaths) { path =>
        ZIO(Source.fromFile(new File(path)))
          .bracket(s => URIO(s.close())) { s =>
            ZIO(s.getLines().mkString)
          }
      }
      count: List[Int] <- ZIO.foreach(filesData) { data =>
        wordCounter(data)
      }
...
object ZioFileReaderFiber extends zio.App {
  ...
  val fileWordCount: ZIO[Console, Throwable, Unit] = for {
    filesData: Fiber.Runtime[Throwable, List[String]] <- ZIO
      .foreach(filePaths) { path =>
        ZIO(Source.fromFile(new File(path)))
    ...
    count: Fiber.Runtime[Throwable, List[Int]] <- ZIO
      .foreach(v1) { data =>
        wordCounter(data)
      }
      .debug(printThread)
